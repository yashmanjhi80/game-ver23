  ...
  useEffect(() => {
    const checkExistingLogin = () => {
      const storedCredentials = localStorage.getItem(
        getStorageKey("USER_CREDENTIALS"),
      );
  ...
  useEffect(() => {
    if (backgroundMusicRef.current && isFeatureEnabled("MUSIC_ENABLED")) {
      backgroundMusicRef.current.volume =
        APP_CONFIG.AUDIO.BACKGROUND_MUSIC_VOLUME;
    }
  }, []);
    ...
    if (backgroundMusicRef.current && isFeatureEnabled("MUSIC_ENABLED")) {
      if (isMusicMuted) {
        ...
        backgroundMusicRef.current
          .play()
          .catch((e) => console.error("Music play prevented:", e));
      } else {
  ...
  const handleSendVerificationCode = async () => {
    ...
    try {
      const response = await fetch(
        "https://game.zyronetworks.shop/send-verification-code",
        {
          method: "POST",
    ...
    <div className="relative min-h-screen w-full overflow-hidden bg-gradient-to-br from-black via-gray-900 to-black">
      ...
      {isFeatureEnabled("MUSIC_ENABLED") && (
        <audio
          ...
          src={APP_CONFIG.AUDIO.ASSETS.BACKGROUND_MUSIC}
          loop
          muted={isMusicMuted}
        />
      )}
      ...
      <audio
        ...
        src={APP_CONFIG.AUDIO.ASSETS.BUTTON_CLICK}
      />
            ...
            <div className="w-full max-w-md">
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                  ...
                  <div className="relative">
                    <div className="relative w-full h-12 flex items-center px-6 bg-black/40 border border-yellow-500/30 rounded-lg backdrop-blur-sm">
                      <CircleUserRound
                        size={26}
                        className="bg-red-400/ z-30 mr-4 text-yellow-400 "
                      />
                      <input
                        type="text"
                  ...
                  <div className="mt-8 text-center space-y-4">
                    ...
                    <button
                      ...
                      disabled={isLoading}
                    >
                      <span className="text-black  font-bold drop-shadow-lg">
                        CREATE ACCOUNT
                      </span>
                    </button>
                  </div>
              ...
              <div className="mt-8 text-center">
                <p className="text-yellow-300/70 text-sm">
                  © 2026 AURA7. All rights reserved.
                </p>
              </div>
            </div>
          </div>
